...
namespace Transformer.Core.Template
{
    ...
    public class VariableUsage
    {
        ...
        private void ParseUsage(string usage, IEnumerable<Variable> variables)
        {
            ...
            if (!usage.Contains(":"))
            {
                var envValue = System.Environment.GetEnvironmentVariable(usage);
                if (envValue != null) Log.WarnFormat("Variable {0} value has been overridden by environment variable.", usage);
                Variable = envValue != null ? Variable = new Variable() { Name = usage, Value = envValue } : variables.FirstOrDefault(v => v.Name == usage);
